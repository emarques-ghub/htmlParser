          ...
          if (/(<\/TR>)/.test(linha))
            break;
              ...
              if (/(^[0-3][0-9]\/[0-1][0-9])/.test(linha)) {
                  transacao.data = />(^[0-3][0-9]\/[0-1][0-9])/.exec(linha)[0].substr(1) 
              }
              if (/(- )?(\d+\.)?\d+(\,\d{1,2})/.test(linha)) {
                transacao.valor = /(- )?(\d+\.)?\d+(\,\d{1,2})/.exec(linha)[0];
              }    
              if (/(<P.*>)(.*)(<\/P>)/.test(linha)) {
                transacao.memo = linha.match('<P.*>' + "(.*)" + '</P>')[1];
              } 
          }
        } ;
        //criarElemento(transacao)
        if (/^([0-3][0-9]\/[0-1][0-9])/.test(transacao.data)) criarElemento(transacao.data + " | " + transacao.memo + " | " + transacao.valor);
      }
